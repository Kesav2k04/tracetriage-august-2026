...
| IBM Carbon | The console's design system: the Gray 100 lightness ramp, the type scale, the 8px spacing steps and the productive motion curves, written as custom properties rather than pulled in as a component library | `apps/web/app/globals.css` |
| IBM Plex | Sans and Mono, self-hosted, latin subset, three weights. No font is requested from another origin | `apps/web/app/layout.tsx`, `@fontsource/ibm-plex-sans`, `@fontsource/ibm-plex-mono` |
...
The Carbon neutrals are re-expressed in OKLCH at the same lightness with a small chroma at
hue 264, which leaves every contrast ratio where Carbon put it: 16.45:1 becomes 16.42:1 on
the page ground and 4.56:1 becomes 4.55:1 on a tile. `scripts/check_contrast.py` recomputes
every pair from that file and `tests/test_contrast.py` fails the suite if one drops below
its floor.
