...
Three minutes, narrated, and every figure spoken in it is read at build time from a receipt
in this repository rather than typed into a script. A second model transcribes the finished
audio without being shown the script, and each figure is looked for in what it heard;
`artifacts/NARRATION_RECEIPT.json` records what was said, what was heard, and whether the
two agreed.

It is published as a link rather than committed. The render is a 4K mp4 tens of megabytes
long and the narration is a voice recording, and a clone needs neither in order to check a
number. `artifacts/FILM_RECEIPT.json` records its exact size and digest.

<!-- generated by scripts/sync_film_chapters.py: film chapters, do not edit -->

The cut runs 2:57 across 14 beats, under the challenge's three-minute cap.

```
0:00 One pass, one recording, and everything written about it
0:16 What TraceTriage decides
0:25 Why most of the archive was never read
0:36 Measuring the Doppler offset instead of guessing it
1:03 Ordering the queue by review value
1:12 A live take on the deployed console
1:26 The committed LangFlow run, and what it refused
1:43 The grounding checker that decides what ships
1:51 Six gates, with thresholds written down first
2:09 The point estimate and its interval
2:19 The three results that came back decided
2:33 What IBM Bob built
2:44 Receipts, licence and sources
2:51 Close
```
...
LICENSE
README.md
api/   (4 files)                              the two Python functions the live measurement runs on
apps/
...
contracts/   (8 files)                        JSON Schemas the data plane is validated against
docs/   (19 files)                            pre-registrations, the kill-gate log, and the build log
flows/   (2 files)                            LangFlow graphs, committed as files LangFlow wrote and can re-import
mobile/   (17 files)                          the Expo client
pipeline/
  tracetriage/   (31 files)                   the measurement path: corridor fit, queue, baselines, grounding
presentation/                                 the film, rendered from the same receipts the console serves
  narration/   (2 files)
  scripts/   (2 files)
  src/   (25 files)
  test/   (2 files)
scripts/   (78 files)                         one runnable step each, and the gate that ties them together
...
<img src="docs/architecture.gif" alt="Thirteen pipeline stages lighting up in the order they run, beside a panel naming the one currently running. A frozen SatNOGS snapshot feeds SGP4 physics, then four frozen splits. Two evidence channels run side by side: image processing into features, and a bounded corridor fit into a calibrated image-only baseline arm. Both feed a small calibrated fusion head, then calibration with out-of-distribution detection and abstention, then the review-value queue. The queue feeds a Granite reviewer note and a read-only evidence agent, and both feed the static console. Every card names the receipt its stage writes, and the panel names what that stage reads from, what it feeds, and the module that implements it." width="904">
