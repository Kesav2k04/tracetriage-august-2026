...
| Do the tests pass offline? | `pytest -m "not network and not ocr and not llm" -q` | 3 failed, 1727 passed, 41 skipped, measured in a clean clone of `3fae24b` with every non-loopback socket refused |
...
`tests/test_readme_claims.py::test_every_path_the_readme_names_exists_and_is_not_empty[apps/web/lib/data]`.
The transcript is the record of commit `3fae24b` and of nothing later, so what this page
...
fresh clone of commit `3fae24b` with every non-loopback socket refused: 13 of 16 steps
succeeded. What did not: offline test suite, snapshot present, offline test suite,
snapshot HIDDEN, lint (ruff check .). The transcript carries each step's exit code and the
