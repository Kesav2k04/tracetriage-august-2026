...
| IBM Bob | The build, and then the product | 13 dated units in `docs/BOB_BUILD_LOG.md`, each with what failed before it was accepted. Bob also operates the finished system: `docs/BOB_DEMO.md` is one paste that ranks the queue, refuses an invented frequency, measures a pass recorded in the last hour and then refuses a sentence about that measurement |
