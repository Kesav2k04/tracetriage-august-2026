...
At this commit: 41 JSON artifacts, 8 contracts, 59 scripts, 29 package modules and 1436 test functions across 79 test modules. Parametrised functions collect as more than one case, so pytest's collected count is higher.
...
| `CLEAN_CLONE_TRANSCRIPT.json` | CLEAN_CLONE_TRANSCRIPT | 0.1.0 | none | 15,407 | `ef0272079771e5ad` | `scripts/check_receipt_digests.py`, `scripts/clean_clone_check.py`, `scripts/sync_for_judges.py` | `test_clean_clone.py`, `test_for_judges.py` |
