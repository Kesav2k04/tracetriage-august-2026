...
are met, three came back inconclusive, one was never run:
  ...
  3 corridor intersects a visible trace NOT_ESTABLISHED. All 3 testable observations
                                        ...
                                        is 0.368. Withdrawn from PASSED on 18 Aug.
...
and all three are fixed. Know about them, because the fix changes how you must work:
  ...
  - KILL_GATE.md published two different confidence intervals for gate 6 in one file.
    ...
    Do not hand-edit those sections. Re-run the script after any re-run of the
    pipeline, and run it with --check in any gate you add: it exits 1 on drift and
    writes nothing. The first version of that script could not run twice, which is
    recorded in the KILL_GATE.md failure log and fixed; tests/test_kill_gate_sync.py
    now asserts idempotence and asserts each generated row against its receipt.
