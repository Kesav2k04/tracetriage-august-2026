...
| | |
|---|---|
| **Handoff written** | 2026-08-20, IST, after D15i, which closed the last of the council check's findings. It scored the submission from four blind judge seats against the four criteria the Official Rules define, and the internal release gate came back **not met** at a mean of 15.5 of 20 against a bar of 18. Every defect the seats found is closed; the score is recorded as it fell |
| **Waves completed** | **A** (A0 to A7), **B** (B1 to B6), **C** (C1 to C7h), **D** (D0 to D15i), **E** (E0 to E8) |
...
| **What remains** | Submission steps, none of them in the tree: the IBM SkillsBuild learning activity, the demo video recorded against `docs/DEMO_SCRIPT.md`, and the challenge submission page. Nothing in the tree is waiting on any of them. The running status of all three is tracked outside this repository, because a document that reasons about how judges score belongs somewhere other than the entry. |
...
| **Open failures** | none. **1,319** offline tests collected, 4 deselected by the marker expression, **1,315** selected and all passing, no expected failures. The three numbers are published together because a single count of a run with a deselection in it does not say which one it is. Lint clean. **113** console tests pass under vitest across 8 files. `npx tsc --noEmit` clean. `npx next build` emits **32** `index.html` files plus a 404 and an icon. `scripts/gate.py` green on **19 of 19** standing gates. `artifacts/SIGNOFF_RECEIPT.json` reads SIGNED at `4d380b4` over 10 checks, 10 passed, none failed and none not checked, including the deployed console at HTTP 200. |
...
40 build credits is per trial account, not a project ceiling, and the procedure for what to do when one runs out is kept outside this repository along with the budget: it is build logistics rather than evidence about the product. What costs you is a bad handoff, so the stop-at-3-credits rule and the checklist below are the parts that matter.
