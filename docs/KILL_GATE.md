...
**Gate 3 is NOT_ESTABLISHED.** 3 of 3 testable observations discriminate (100%) and 2 of 2 independent (station, date) groups do, and neither rate establishes the 70% threshold. The exact one-sided Clopper-Pearson 95% lower bound is `0.3684` over observations and `0.2236` over groups, so the data are consistent with a true intersection rate well below the bar. A perfect rate would need 9 of 9 to clear 0.70. The sample is what fails here, not the measurements: the per-observation evidence is below and is unchanged by this.
<!-- end gate 3 verdict -->
...
|---|---|---|---|---|---|---|
| A3's decisive set | 3 | 3 | 100% | 0.3684 | NOT_ESTABLISHED | A3's verdict word, before E16 split the pools |
| A, corridor-selected | not run | | | | | A3's `UNCORRECTED` label, which reads the corridor |

The published receipt is still A3's decisive set, chosen before `docs/E16_PREREGISTRATION.md` split the pools. Neither pre-registered pool has been scored yet, so there is no comparison to draw and this table says so rather than presenting one pool's rate as both.
<!-- end gate 3 pools -->
