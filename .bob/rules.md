...
Already proven, so build on it rather than re-testing it: the observation's own stored TLE plus station coordinates reproduce pass geometry. The corpus figure is a **median of 0.22 degrees** against the API's own `max_altitude`, over 199 successes in `artifacts/PHYSICS_VALIDATION.json`. The 0.18 this line used to give is one observation, 14513023, at 31.18 degrees computed against 31.0 reported (`docs/SATNOGS_API_RECON.md:230`), and it read as a corpus result under a heading that tells you not to re-derive it.
